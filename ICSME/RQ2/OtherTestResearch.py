...
from Dataset.Repository import NonTrivialRepo
...
class OtherTestResearch:
    ...
    @staticmethod
    def filter_java_repo(list):
        results = []
        for repo_name in list:
            repo = OtherTestResearch.get_repo_by_name(repo_name)
            if repo.is_web_java == 1:
    ...
    @staticmethod
    def filter_js_ts_repo(list):
        results = []
        for repo_name in list:
            repo = OtherTestResearch.get_repo_by_name(repo_name)
            if repo.is_web_javascript == 1 or repo.is_web_typescript == 1:
                results.append(repo_name)
    ...
    @staticmethod
    def filter_py_repo(list):
        results = []
        for repo_name in list:
            repo = OtherTestResearch.get_repo_by_name(repo_name)
            if repo.is_web_python == 1:
                results.append(repo_name)
    ...
    def search_other_kind_of_test_java(start,end):
        missed = OtherTestResearch.get_unprocessed_repo_icsme()
        print(len(missed))
        repos = OtherTestResearch.filter_java_repo(missed)
        path_folder_clone = f"/home/sergio/PycharmProjects/E2E-Miner-A-tool-for-mining-E2E-tests-from-software-repositories/clone"
        for repo_name in repos[start:end]:
            if repo_name not in OtherTestResearch.black_list:
                repo_name
                ...
                try:
                    df.to_csv(f'/home/sergio/PycharmProjects/E2E-Miner-A-tool-for-mining-E2E-tests-from-software-repositories/ICSME/RQ2/other_test_analysis/java_web_app/'+new_name+'.csv', index=False)
    ...
    def search_other_kind_of_test_jsts(start,end):
        missed = OtherTestResearch.get_unprocessed_repo_icsme()
        repos = OtherTestResearch.filter_js_ts_repo(missed)
        path_folder_clone = f"/home/sergio/PycharmProjects/E2E-Miner-A-tool-for-mining-E2E-tests-from-software-repositories/clone"
        for repo_name in repos[start:end]:
            if repo_name not in OtherTestResearch.black_list:
                cloner = Cloner(path_folder_clone)
                    ...
                    for file in files:
                        if file.endswith(".js") or file.endswith(".ts"):
                            ...
                            if is_test:
                                ...
                                df = pd.concat([df, new_row], ignore_index=True)
                try:
                    df.to_csv(f'/home/sergio/PycharmProjects/E2E-Miner-A-tool-for-mining-E2E-tests-from-software-repositories/ICSME/RQ2/other_test_analysis/js_ts_web_app/'+new_name+'.csv', index=False)
                    print(f'file saved {new_name+".csv"}!')
                except Exception as e:
                    print(f'Error: {e}')
                # clear clone directory
    ...
    def search_other_kind_of_test_python(start,end):
        missed = OtherTestResearch.get_unprocessed_repo_icsme()
        print(len(missed))
        repos = OtherTestResearch.filter_py_repo(missed)
        path_folder_clone = f"/home/sergio/PycharmProjects/E2E-Miner-A-tool-for-mining-E2E-tests-from-software-repositories/clone"
        for repo_name in repos[start:end]:
            if repo_name not in OtherTestResearch.black_list:
                cloner = Cloner(path_folder_clone)
                    ...
                    for file in files:
                        if file.endswith(".py"):
                            ...
                            if is_test:
                                ...
                                df = pd.concat([df, new_row], ignore_index=True)
                try:
                    df.to_csv(f'/home/sergio/PycharmProjects/E2E-Miner-A-tool-for-mining-E2E-tests-from-software-repositories/ICSME/RQ2/other_test_analysis/py_web_app/'+new_name+'.csv', index=False)
                    print(f'file saved {new_name+".csv"}!')
                except Exception as e:
                    print(f'Error: {e}')
                # clear clone directory
    ...
    @staticmethod
    def get_unprocessed_repo_icsme():
        missed_repo = []
        ...
        for index, row in df.iterrows():
            ...
            if pd.isna(df.at[index, "nOtherTest"]):
                missed_repo.append(repo_name)
        return missed_repo


    @staticmethod
    def calculate_num_other_test(repo_name):
        new_name = repo_name.replace('/', '_')
        java_file = f'/home/sergio/PycharmProjects/E2E-Miner-A-tool-for-mining-E2E-tests-from-software-repositories/ICSME/RQ2/other_test_analysis/java_web_app/' + new_name + '.csv'
        py_file = f'/home/sergio/PycharmProjects/E2E-Miner-A-tool-for-mining-E2E-tests-from-software-repositories/ICSME/RQ2/other_test_analysis/py_web_app/' + new_name + '.csv'
        js_ts_file = f'/home/sergio/PycharmProjects/E2E-Miner-A-tool-for-mining-E2E-tests-from-software-repositories/ICSME/RQ2/other_test_analysis/js_ts_web_app/'+new_name+'.csv'
        num_test = 0
        if os.path.exists(java_file):
            df = pd.read_csv(java_file)
            total_tests = df['num of tests'].sum()
            num_test+=total_tests
        if os.path.exists(js_ts_file):
            df = pd.read_csv(js_ts_file)
            total_tests = df['num of tests'].sum()
            num_test += total_tests
        if os.path.exists(py_file):
            df = pd.read_csv(py_file)
            total_tests = df['num of tests'].sum()
            num_test += total_tests
        return num_test
    ...
    @staticmethod
    def fill_num_other_test():
        file_path = '/home/sergio/PycharmProjects/E2E-Miner-A-tool-for-mining-E2E-tests-from-software-repositories/ICSME/RQ2/rq2_data.xlsx'
        df = pd.read_excel(file_path)
        for index, row in df.iterrows():
            repo_name = row["repoName"]
            if pd.isna(df.at[index, "nOtherTest"]):
                num_test = OtherTestResearch.calculate_num_other_test(repo_name)
                df.at[index,"nOtherTest"] = num_test
...
if __name__ == "__main__":
    ...
    print(len(repos))
    '''
    OtherTestResearch.fill_num_other_test()